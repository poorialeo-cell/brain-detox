...
import {
  ...
  SafeAreaView, StatusBar, Animated, Easing, Dimensions,
} from 'react-native';
...
export default function BrainRotTestScreen({ navigation }: Props) {
  ...
  }, [applyTestResult, resultOpacity, resultScale]);

  const advanceQuestion = useCallback((newAnswers: number[], nextIdx: number) => {
    if (nextIdx < TOTAL_Q) {
      translateX.setValue(width);
      setCurrentIdx(nextIdx);
      setIsAnimating(false);
      translateX.value = withTimingJS(translateX, 0, 280);
    } else {
      setIsAnimating(false);
      handleFinish(newAnswers);
    }
  }, [translateX, handleFinish]);
  ...
  const handleAnswer = useCallback((answerIdx: number) => {
    ...
    Animated.timing(translateX, {
      ...
      if (finished) advanceQuestion(newAnswers, currentIdx + 1);
    });
  }, [isAnimating, answers, currentIdx, translateX, haptics, advanceQuestion]);
          ...
          <>
            ...
            <View style={styles.progressContainer}>
              <View style={styles.progressLabelRow}>
                ...
                <Text style={styles.progressCount}>{currentIdx + 1}<Text style={styles.progressTotal}> / {TOTAL_Q}</Text></Text>
              </View>
...
// Animated.timingをPromiseなしに即実行するヘルパー
function withTimingJS(anim: Animated.Value, toValue: number, duration: number): NodeJS.Timeout {
  anim.setValue(anim); // no-op
...
const styles = StyleSheet.create({
  ...
  questionChip: {
    ...
    backgroundColor: '#1e1433', borderWidth: 1, borderColor: '#a78bfa',
  ...
  resultDelta: { fontSize: 52, fontWeight: '900' },
